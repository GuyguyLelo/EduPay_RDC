...
try:
    ...
    logger.warning("Le SDK CinetPay n'est pas installé. Installez-le avec: pip install -i https://test.pypi.org/simple/ cinetpay-sdk==0.1.1")
...
class CinetPayService:
    ...
    def __init__(self):
        """Initialise le service CinetPay avec le SDK officiel"""
        if not CINETPAY_SDK_AVAILABLE:
            raise ImportError(
                "Le SDK CinetPay n'est pas installé. "
                "Installez-le avec: pip install -i https://test.pypi.org/simple/ cinetpay-sdk==0.1.1"
            )
        ...
        try:
            ...
            logger.error(f"Erreur lors de l'initialisation du client CinetPay: {e}")
            raise ValueError(f"Impossible d'initialiser le client CinetPay: {str(e)}")
    ...
    def initier_paiement_mobile_money(self, paiement: Paiement, numero_telephone: str, operateur: str):
        ...
        try:
            ...
            # Important: Inclure le numéro de téléphone pour que CinetPay envoie le SMS de confirmation
            data = {
                'amount': amount,
            ...
            # selon l'opérateur. Le numéro de téléphone est essentiel pour recevoir le SMS de confirmation.
            
            # Initialiser le paiement avec le SDK
            response = self.client.PaymentInitialization(data)
            
            logger.info(f"Réponse CinetPay pour paiement {paiement.id}: {response}")
